...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            ...
            $stmt = $conn->prepare("INSERT INTO students (name, email, date_of_birth, course, grade, profile_picture) VALUES (?, ?, ?, ?, ?, ?)");
            if ($stmt === false) {
                // Error handling for prepare statement [cite: 11]
            ...
            try { // Error and Exception Handling [cite: 11]
                if ($stmt->execute()) {
                    ...
                    logAction("Student added: " . $name . " (Email: " . $email . ")"); // Log student addition 
                } else {
